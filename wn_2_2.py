...
class BoxLayoutExample(BoxLayout):
        def __init__(self, **kwargs):
        	...
        	d = DrawInput()
        	print(f"box {d.numx}")
        	self.label = Label(text=f"{d.numx} ", size_hint= (1, 0.2), font_size= '100dp')
        	print("label = ", self.label.text)
        	d.bind(numx=self.update_label)
        	self.add_widget(self.label)
        	self.add_widget(d)
        	b=Button(text='Submit', size_hint=(1, 0.1), font_size='30dp')
        	self.add_widget(b)
        	b.bind(on_press=d.submit)
...
class DrawInput(Widget):
    ...
    def submit(self, instance):
        self.export_to_png(f'images/{self.numx}/no_{self.numx}_s{self.rser}.png')
        self.wipe()
        self.gen_num()
